...
cot_proy <- data.frame(fecha = seq.Date(as.Date("2025-04-14"), as.Date("2025-12-31"), by = "day"))
# Define the start and end dates for the two growth periods
start_date_1 <- as.Date("2025-04-14"); end_date_1 <- as.Date("2025-08-31"); start_date_2 <- as.Date("2025-01-01"); end_date_2 <- as.Date("2025-12-31")
# Define the initial and final values for each period
initial_value_1 <- 1250; final_value_1 <- 1350 ; final_value_2 <- 1500
...
meses<-seq.Date(as.Date("2025-04-01"), as.Date("2025-12-31"), by = "month")
cdusd<-perc_mes %>% filter(impacto_presupuestario_mes>=4) %>% pull(perc) *t2025
...
anual_data<- data%>% filter(fecha < as.Date("2025-04-01")) %>% group_by(fecha, impacto_presupuestario_anio) %>% summarise(credito_devengado_usd=sum(credito_devengado_usd))
anual_data_proy<-rbind(anual_data,proy2025)
...
mensual <- data %>% filter(fecha < as.Date("2025-04-01")) %>% group_by(fecha,impacto_presupuestario_anio) %>% summarise(credito_devengado_usd=sum(credito_devengado_usd))
